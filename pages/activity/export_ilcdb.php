...
$filterSector = isset($_GET['subproject']) ? mysqli_real_escape_string($con, $_GET['subproject']) : '';
...
$query = "SELECT 
            ...
            district, agency, indicator, subproject, person, resource, participants, completers, 
            male, female, approved, mov, remarks 
          FROM tblactivity 
          WHERE project = 'ILCDB'"; // Start with a base query that retrieves all records
...
if (!empty($filterSector)) {
    $query .= " AND subproject = '$filterSector'";
}
...
header('Content-Disposition: attachment; filename=DICT-SDN ILCDBsecurity Activity Data.csv');
...
fputcsv($output, array(
    ...
    'Barangay', 'District', 'Agency', 'indicator', 'subproject', 'Person', 'Resource', 
