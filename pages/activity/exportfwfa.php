...
$filterSubproject = isset($_GET['sector']) ? mysqli_real_escape_string($con, $_GET['sector']) : '';
...
$query = "SELECT 
          ...
          FROM tblactivity 
          WHERE project = 'FWFA'"; // Start with a base query that retrieves all records

// Apply filters if provided
if (!empty($filterMode)) {
    $query .= " AND mode = '$filterMode'";
}
if (!empty($filterSubproject)) {
    $query .= " AND sector = '$filterSubproject'";
}
...
header('Content-Disposition: attachment; filename=DICT-SDN FWFA Activity Data.csv');
